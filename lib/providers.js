// Provider registry and low-level streaming/completion helpers.
// This file normalizes OpenAI-compatible, Anthropic, and Gemini APIs into a single interface.
const PROVIDERS = {
  groq: {
    id: "groq",
    type: "openai",
...
// Normalize messages for OpenAI-compatible chat endpoints.
...
// Normalize messages for Gemini generateContent/streamGenerateContent.
...
// Apply a hard timeout to fetch calls to avoid hanging requests.
...
// Parse a single SSE event into { event, data }.
...
// Iterate over SSE events from a streaming response body.
...
// Extract incremental token deltas from OpenAI-compatible SSE payloads.
...
// Extract final content from a non-streaming OpenAI-compatible response.
function extractOpenAIContent(payload) {
  const content = payload.choices?.[0]?.message?.content;
  return typeof content === "string" ? content : "";
}

// Extract incremental token deltas from Anthropic SSE payloads.
...
// Extract final content from a non-streaming Anthropic response.
function extractAnthropicContent(payload) {
  const parts = Array.isArray(payload.content) ? payload.content : [];
  return parts.map((p) => p.text || "").join("");
}

// Extract incremental token deltas from Gemini SSE payloads.
function extractGeminiDelta(payload) {
  const parts = payload.candidates?.[0]?.content?.parts || [];
  return parts.map((p) => p.text || "").join("");
}

// Extract final content from a non-streaming Gemini response.
function extractGeminiContent(payload) {
  const parts = payload.candidates?.[0]?.content?.parts || [];
  return parts.map((p) => p.text || "").join("");
}

// OpenAI-compatible streaming (OpenAI + Groq share this format).
...
// Anthropic non-streaming completion via Messages API.
...
// Gemini non-streaming via generateContent.
...
// Unified streaming interface for all supported providers.
...
// Unified non-streaming interface for all supported providers.
