...
function extractSSEData(eventBlock) {
  return eventBlock
    ...
    .map((line) => {
      const raw = line.slice(5); // Remove "data:" prefix
      // Attempt base64 decode (new format). Fall back to raw if not valid base64.
      try {
        // Use atob() for browser-compatible base64 decoding
        return decodeURIComponent(escape(atob(raw.trim())));
      } catch {
        // Fall back to raw if not valid base64 (e.g., legacy plain-text SSE)
        return raw;
      }
    })
    .join("");
